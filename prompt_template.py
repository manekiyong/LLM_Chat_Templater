...
class OpenAITemplate:
    def __init__(self, model: str):
        ...
        model = model.lower()
        all_model_names = conv_templates.keys()
        if model in all_model_names:
            ...
            for name in all_model_names:
                edit_dist = lev_dist(name, model)
                if edit_dist < smallest_dist:
                    smallest_dist = edit_dist
                    model_name = name
            warnings.warn(f"Exact model name not found, fuzzy matched to {model_name}")
            self.model = model_name
        
    def __call__(self, messages: List[str]):
        """
        Generate Prompt, messages follows OpenAI format
        {"role": "system/user/assistant",
        "content": ""}
        """
        if self.model=="chatgpt":
            return messages
        conv = get_conv_template(self.model)
        turn = 'user'
        if messages[0]['role']!='system':
            raise ValueError(f"Conversation chain must start with `system`")
        conv.set_system_message(messages[0]['content'])
        for message in messages[1:]:
            if message['role']!=turn:
                raise ValueError(f"Conversation chain must alternate between user & assistant")
            if message['role'] == 'user':
                role = conv.roles[0]
                turn = 'assistant' # toggle role
            elif message['role'] == 'assistant':
                role = conv.roles[1]
                turn = 'user' # toggle role
            else:
                raise ValueError(f"Invalid role {message['role']}")
            conv.append_message(role, message['content'])
        if turn=='user':
            raise ValueError(f"Last message in list should be from `user`")
